...
const trackingClimbQuestions: Question[] = [
  {
    id: "commercial-performance",
    title:
      "We have a good grasp of our current commercial performance including revenue, gross profit, average deal value",
    type: "likert-scale",
  ...
  {
    id: "revenue-profit-targets",
    title:
      "Everyone, that needs to know, has clarity on what our revenue & profit targets are for this current financial year",
    type: "likert-scale",
    options: ["Strongly Disagree", "Disagree", "N/A", "Agree", "Strongly Agree"],
    additionalInfo: true,
    required: true,
  },
  {
    ...
    title:
      "Our pipeline is managed by stages in a sales funnel, and we can use it to forecast sales for the next 12 months",
    type: "likert-scale",
    options: ["Strongly Disagree", "Disagree", "N/A", "Agree", "Strongly Agree"],
    additionalInfo: true,
    required: true,
  ...
  {
    id: "great-sale-recognition",
    title: "Everyone that is responsible for working with clients recognises what makes a great sale for this business",
    type: "likert-scale",
    options: ["Strongly Disagree", "Disagree", "N/A", "Agree", "Strongly Agree"],
    additionalInfo: true,
    required: true,
  ...
  {
    id: "three-year-targets",
    title: "We have clarity on what our sales & profit targets need to be for the next 3 years to achieve our goals",
    type: "likert-scale",
    options: ["Strongly Disagree", "Disagree", "N/A", "Agree", "Strongly Agree"],
    additionalInfo: true,
    required: true,
  ...
  {
    id: "kpis-metrics",
    title: "We have KPIs or metrics defined at each stage of our sales funnel leading to our ultimate targets",
    type: "likert-scale",
    options: ["Strongly Disagree", "Disagree", "N/A", "Agree", "Strongly Agree"],
    additionalInfo: true,
    required: true,
...
export function TrackingClimbQuestions({ answers, onAnswer }: TrackingClimbQuestionsProps) {
  const [expandedQuestions, setExpandedQuestions] = useState<Set<string>>(new Set([trackingClimbQuestions[0].id]))
  ...
  const handleOptionSelect = (questionId: string, option: string) => {
    ...
    // 自动跳转到下一题
    const currentIndex = trackingClimbQuestions.findIndex((q) => q.id === questionId)
    if (currentIndex < trackingClimbQuestions.length - 1) {
      const nextQuestionId = trackingClimbQuestions[currentIndex + 1].id
      setTimeout(() => {
  ...
  return (
    <div className="space-y-4">
      {trackingClimbQuestions.map((question, index) => {
